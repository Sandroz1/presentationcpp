...
import imgTopicErrors from "@/imports/cropped/uchicode-topic-errors.png";
import imgTopicCheck from "@/imports/cropped/uchicode-topic-check.png";
...
function TopicPreview({ label, imgSrc, accent = C }: { label: string; imgSrc: string; accent?: string }) {
  return (
    <div style={{ ...card({ borderColor: accent + "28", background: "#0B1424" }), flex: 1, minHeight: 0, overflow: "hidden", display: "flex", flexDirection: "column", boxShadow: "0 8px 22px rgba(0,0,0,0.34)" }}>
      <div style={{ padding: "5px 10px", background: accent + "0D", borderBottom: `1px solid ${accent}22`, flexShrink: 0 }}>
        <span style={{ fontSize: 10, fontWeight: 800, color: accent, letterSpacing: "0.04em", textTransform: "uppercase" }}>{label}</span>
      </div>
      <div style={{ flex: 1, minHeight: 0, overflow: "hidden", background: "#F8FAF6" }}>
        <ImageWithFallback
          src={imgSrc}
          alt={label}
          style={{ width: "100%", height: "100%", objectFit: "cover", objectPosition: "top center", display: "block" }}
...
const slides: { content: React.ReactNode; notes: string }[] = [
  ...
  // 9 ─ Пример кода
  {
    ...
    content: (
      <Frame title="Пример кода внутри темы" thesis="Ученик сразу видит, как идея работает в реальном C++ коде." n={9} accent={V}>
        <div style={{ display: "flex", gap: 18, height: "100%" }}>
          <div style={{ flex: 1.6, alignSelf: "center", background: "#0F1A2A", borderRadius: 10, overflow: "hidden", borderWidth: 1, borderStyle: "solid", borderColor: "rgba(255,255,255,0.1)", boxShadow: "0 8px 24px rgba(0,0,0,0.4)", display: "flex", flexDirection: "column" }}>
            ...
            <div style={{ background: "#24262B", overflow: "hidden" }}>
              <ImageWithFallback
                src={imgTheoryCode}
                alt="Пример кода C++ с циклом while и стоп-значением"
                style={{ width: "100%", height: "auto", maxHeight: 318, objectFit: "contain", objectPosition: "top", display: "block" }}
              />
            </div>
          </div>
          <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 10, justifyContent: "center" }}>
  ...
  // 18 ─ Полный текст выступления
  {
    ...
    content: (() => {
      const script = [
        { n: "01", title: "Uchicode", text: "Я представляю uchicode.ru — учебный сайт по C++ и ООП для новичков. Он ведёт ученика по маршруту: курс, тема, пример, задача, ошибки, AI и прогресс." },
        { n: "02", title: "Что такое Uchicode", text: "Uchicode собирает обучение C++ в один сценарий. Ученик идёт по темам, видит пример и сразу понимает, куда перейти дальше." },
        { n: "03", title: "Проблема новичка", text: "Новичку сложно выбрать первый шаг: в C++ быстро появляются термины, ошибки и файлы. Сайт снижает хаос и показывает порядок." },
        { n: "04", title: "Для кого сайт", text: "Аудитория — начинающие, студенты и те, кому нужна практика после теории. Для преподавателя это может быть дополнительный учебный ресурс." },
        { n: "05", title: "Главная страница", text: "Главная страница показывает, что это сайт про C++ и с чего начать. Маршрут до первой задачи виден сразу." },
        { n: "06", title: "Курсы", text: "Курсы задают порядок: сначала база C++, затем ООП. Так ученик не выбирает темы наугад." },
        { n: "07", title: "Курс ООП C++", text: "Страница курса показывает открытые уроки, статус и следующий шаг. Ученик понимает, где находится сейчас." },
        { n: "08", title: "Страница темы", text: "Страница темы даёт одну идею, пример, разбор ошибок и мини-проверку. Теория сразу связана с практикой." },
        { n: "09", title: "Пример кода", text: "Пример кода показывает, как идея работает в реальном C++. Это помогает перейти от чтения к действию." },
        { n: "10", title: "Практические задачи", text: "Задачи закрепляют тему. Статус показывает, что уже сделано и что доступно дальше." },
        { n: "11", title: "Как учиться", text: "Раздел 'Как учиться' объясняет порядок работы с задачей: условие, минимальный код, один шаг, проверка." },
        { n: "12", title: "Частые ошибки", text: "Ошибки разобраны по схеме: что сломалось, почему и как исправить. Это помогает понять причину, а не копировать ответ." },
        { n: "13", title: "AI-советник", text: "AI-советник помогает внутри темы: ученик выделяет фрагмент и задаёт вопрос. Ответ приходит простыми словами по конкретному месту." },
        { n: "14", title: "Профиль и прогресс", text: "Профиль показывает пройденные уроки, решённые задачи и AI-лимит. Это помогает видеть следующий шаг." },
        { n: "15", title: "Что дальше", text: "Дальше можно развивать интерактивную практику: решения на сайте, обратную связь и связь с преподавателем." },
        { n: "16", title: "Итог", text: "Uchicode помогает войти в программирование через C++: понятный маршрут, практика и прогресс." },
        { n: "КР", title: "Если нужно сказать совсем кратко", text: "Коротко: Uchicode — учебный сайт по C++ и ООП для новичков. Он собирает курс, тему, пример, задачу, ошибки, AI и прогресс в один понятный путь." },
      ];
